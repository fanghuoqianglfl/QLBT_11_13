...
%provided correlated strongly, the band of coupling $g(T)$ from using half of data points are close to 1.3 times of results using all lattice data points, while the correlation between parameters in coupling $g(T)$ leads to the change of $g(T)$ and entropy are less than the change of standard deviations of parameters $(a,b,c,d)$}. 
...
%\textcolor{blue}{Due to insufficient events for heavy quarks for hadronization, $v_2$ for AuAu collision fluctuates up and down in Fig. 5. We remind that we didn't include interpolation error of emulator in this work, which lead to the extremely narrow bands of $R_{AA}$ and $v_2$.}
